...
function pack_via_metadata(return_type) {
    return new Promise(function (ok_callback, err_callback) {
        ...
        if (return_type === 'coco') {
            ...
            let result = {};
            for (let filename in _via_img_metadata) {
                let obj = JSON.parse(JSON.stringify(_via_img_metadata[filename]));
                let fname = obj.filename.split("/");
                fname = fname[fname.length - 1];
                obj.filename = fname;
                result[fname] = obj;
            }
            ok_callback([JSON.stringify(result)]);
        }
    }.bind(this));
}
...
function project_save_confirmed(input) {
    ...
    let data = JSON.stringify(_via_project);
    data = JSON.parse(data);

    let result = {};
    for (let filename in data._via_img_metadata) {
        let obj = JSON.parse(JSON.stringify(data._via_img_metadata[filename]));
        let fname = obj.filename.split("/");
        fname = fname[fname.length - 1];
        obj.filename = fname;
        result[fname] = obj;
    }
    data._via_img_metadata = result;
    delete data._via_image_id_list;
    data = JSON.stringify(data);

    async function _save() {
        await API.call("save", { data: data });
...
function image_grid_content_append_img(img_grid_index) {
    ...
    e.src = _via_img_metadata[img_id].filename
...
//         let { code, data } = await API.call("files");
//         if (!data) return;
//         for (let i = 0; i < data.length; i++)
//             data[i] = API.url("download/" + data[i]);
//         let input = { url: { value: '' }, url_list: {} };
//         input.url_list.value = data.join("\n");
//         project_file_add_url_input_done(input);
//     }

//     async function annotations() {
//         let { code, data } = await API.call("annotations");
//         if (code != 200) return;

//         data = JSON.parse(data);
//         let reformat = {};
//         for (let key in data._via_img_metadata) {
//             let obj = data._via_img_metadata[key];
//             obj.filename = API.url("download/" + obj.filename);
//             reformat[obj.filename + "-1"] = obj;
//         }
//         data._via_img_metadata = reformat;
//         data = JSON.stringify(data);
